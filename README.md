LightGCN, xGCN, and baseline recommenders for implicit-feedback datasets with plotting and data utilities.

What’s here
- Models: LightGCN, xGCN, NGCF (simplified), MF-BPR, weighted MF, Mult-VAE.
- Evaluation: Recall@20/50 and NDCG@20/50 on leave-one-out splits; per-epoch histories and plots.
- Utilities: dataset downloader (`data.py`), bipartite subgraph visualizer (`bipartite_graph.py`), interaction heatmap generator (`heatmap.py`).

Quick start
```
pip install -r requirements.txt
python models.py --dataset amazon --model lightgcn --epochs 50 --eval-every 5
```

Run every model with subsetting and plots
```
python models.py --dataset amazon --model all --max-users 200 --max-items-per-user 50 \
...
- Example: `python models.py --dataset gowalla --model lightgcn --layers 3 --edge-dropout 0.1 --neg-sampler popularity --epochs 100`
...
| `--model {lightgcn,xgcn,ngcf,mf-bpr,wmf,mult-vae,all}` | `all` | Leave unset or use `all` to run every model in order. |
| `--embed-dim INT` | `64` | Embedding / latent dimension. |
| `--layers INT` | `3` | Propagation layers (LightGCN/xGCN/NGCF). |
